...
const Hero: FunctionComponent<HeroProps> = (props) => {
    ...
    const rContract = new web3.eth.Contract(dividendContractAbi, dividendContractAddres);
    const rsContract = new web3.eth.Contract(contractAbi, contractAddres);

    const contract = new ethers.Contract(contractAddres, contractAbi, signer);
    //const rcontract = new ethers.Contract(contractAddres, contractAbi);
    ...
    const getTotalClaimed = async () => {
        if (account) {
            ...
            var totalClaim = await rContract.methods.getTotalClaimedDividends(account).call();
    ...
    const [oldAcc, setOldAcc] = useState(account);
    var prevacc;
    ...
    const getOwned = async () => {
        if (account) {
            ...
            var ownedPercent = ((1 * availableSpecies[3] + 1.3 * availableSpecies[2] + 1.6 * availableSpecies[1] + 2 * availableSpecies[0]) / totalStonkerYield) * 100;
    ...
    }, [account, claimable, ownerPercent]); // <-- empty dependency array
